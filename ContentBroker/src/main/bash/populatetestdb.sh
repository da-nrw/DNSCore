...
then
	sqls=(
		...
		"INSERT INTO users (id,short_name,username,password,accountlocked,accountexpired,passwordexpired,enabled,mails_pooled,delta_on_urn,use_public_mets,use_virus_scan,email_contact,friendly_file_exts,provider_type) values (1,'TEST','TEST','\$2a\$10\$CcMH2fhJrHTKzgpGRusvEulQZZPRdBR3l8zSG5QoNmH1HPEvQZM9G',FALSE,FALSE,FALSE,TRUE,TRUE,TRUE,FALSE,FALSE,'Heino','xmp;txt;xml','Bibliothek');"
        "INSERT INTO users (id,short_name,username,password,accountlocked,accountexpired,passwordexpired,enabled,mails_pooled,delta_on_urn,use_public_mets,use_virus_scan,email_contact) values (4,'rods','rods','\$2a\$10\$0A8khxeN56JY6WkUXCNG/uUU7cZsdLHiz616TpLajiLskW/Vr8u8q',FALSE,FALSE,FALSE,TRUE,FALSE,FALSE,FALSE,FALSE,'Heino');"
        "INSERT INTO users (id,short_name,username,password,accountlocked,accountexpired,passwordexpired,enabled,mails_pooled,delta_on_urn,use_public_mets,use_virus_scan,email_contact) values (5,'CI_ADMIN','CI_ADMIN','\$2a\$10\$EblZqNptyYvcLm/VwDCVAuBjzZOI7khzdyGPBr08PpIi0na624b8.',FALSE,FALSE,FALSE,TRUE,FALSE,FALSE,FALSE,FALSE,'Heino');"
        "INSERT INTO users (id,short_name,username,password,accountlocked,accountexpired,passwordexpired,enabled,mails_pooled,delta_on_urn,use_public_mets,use_virus_scan,email_contact) values (6,'CI_ADMIN','N2_ADMIN','\$2a\$10\$EblZqNptyYvcLm/VwDCVAuBjzZOI7khzdyGPBr08PpIi0na624b8.',FALSE,FALSE,FALSE,TRUE,FALSE,FALSE,FALSE,FALSE,'Heino');"
		"INSERT INTO users (id,short_name,username,password,accountlocked,accountexpired,passwordexpired,enabled,mails_pooled,delta_on_urn,use_public_mets,use_virus_scan,email_contact,friendly_file_exts) values (7,'CIRegressionTEST','CIRegressionTEST','\$2a\$10\$Sv3qPUMM20S1Z1eh0Z92OevuNQJKua/k3ZWVVQxAkKsyaneDdYdeW',FALSE,FALSE,FALSE,TRUE,TRUE,TRUE,FALSE,FALSE,'Heino','xmp;txt;xml');"
		...
		"INSERT INTO preservation_system (id,urn_name_space,pres_server,open_collection_name,closed_collection_name,uris_aggr,uris_cho,uris_file,min_repls,uris_local,admin_id,license_validation,license_validation_test_csn) VALUES (1,'urn:nbn:de:danrw','localnode','collection-open','collection-closed','http://data.danrw.de/aggregation','http://data.danrw.de/cho','http://data.danrw.de/file',1,'info:',5,1,1);"
		"INSERT INTO nodes (id,urn_index,name,identifier,admin_id,psystem_id) values (1,0,'localnode','ci',5,1);"
		"INSERT INTO nodes (id,urn_index,name,identifier,admin_id,psystem_id) values (2,0,'cooperatingnode','ci/CN',6,1);"	
	    ...
	    "INSERT INTO conversion_routines (id,name,target_suffix,type,params,psystem_id) VALUES (6,'LZA_PDF','pdf','de.uzk.hki.da.convert.CLIConversionStrategy','gs -q -dPDFA -dPDFACompatibilityPolicy=1 -dBATCH -dNOPAUSE -dNOOUTERSAVE -sColorConversionStrategie=CMYK -sProcessColorModel=DeviceCMYK -sDEVICE=pdfwrite -sOutputFile=output conf/PDFA_def.ps input',1);"
    ...
    #    "INSERT INTO conversion_policies (id,psystem_id,source_format,conversion_routine_id,presentation,format_type) VALUES (8,1,'fmt/43',3,false,'LZA');"
        ...
        "INSERT INTO conversion_policies (id,psystem_id,source_format,conversion_routine_id,presentation,format_type) VALUES (12,1,'fmt/276',6,false,'LZA');"
