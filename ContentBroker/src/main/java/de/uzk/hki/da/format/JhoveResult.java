...
public class JhoveResult{
	...
	public static JhoveResult parseJHoveXML(String outFile)
		...
		try {
			...
			String formatString=repInfo.get(0).getChildElements("format","http://hul.harvard.edu/ois/xml/ns/jhove").get(0).getChild(0).getValue();
			ret.setFormat(formatString);
			StringBuilder messagesStringBuilder=new StringBuilder();
			
			Elements messages=repInfo.get(0).getChildElements("messages","http://hul.harvard.edu/ois/xml/ns/jhove").get(0).getChildElements("message","http://hul.harvard.edu/ois/xml/ns/jhove");
	...
	public boolean isValid(){
		return status.equals(status.equalsIgnoreCase(StatusWellFormedAndValid));
	}

	@Override
	public String toString() {
		return "JHoveResult [status=" + status + ", format=" + format + (message==null?"":", message=" + message) + "]";
	}
