...
<html>
	...
	<body>
		<a href="#show-userRole" class="skip" tabindex="-1">
			...
			<ol class="property-list userRole">
				<g:if test="${userRoleInstance?.role}">
					<li class="fieldcontain">
						<span id="role-label" class="property-label">
				...
				<g:if test="${userRoleInstance?.user}">
					<li class="fieldcontain">
						...
						<span class="property-value" aria-labelledby="user-label">
							<g:link controller="user" action="show" id="${userRoleInstance?.user?.id}">${userRoleInstance?.user?.encodeAsHTML()}</g:link>
						</span>
					</li>
				</g:if>
			</ol>
