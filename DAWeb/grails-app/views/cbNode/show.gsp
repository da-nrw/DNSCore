...
<html>
	...
	<body>
		<a href="#show-cbNode" class="skip" tabindex="-1">
		...
		<div id="show-cbNode" class="content scaffold-show" role="main">
			...
			<ol class="property-list cbNode">
				<g:if test="${cbNodeInstance?.contractors}">
					<li class="fieldcontain">
						<span id="contractors-label" class="property-label">
						...
						<g:each in="${cbNodeInstance.contractors}" var="c">
							<span class="property-value" aria-labelledby="contractors-label">
								<g:link controller="user" action="show" id="${c.id}">${c?.encodeAsHTML()}</g:link>
							</span>
						</g:each>
					</li>
				</g:if>
				<g:if test="${cbNodeInstance?.name}">
					<li class="fieldcontain">
						...
						<span class="property-value" aria-labelledby="name-label">
							<g:fieldValue bean="${cbNodeInstance}" field="name"/>
						</span>
					</li>
				</g:if>
			</ol>
			<g:form url="[resource:cbNodeInstance, action:'delete']" method="DELETE">
				<fieldset class="buttons">
					<g:link class="edit" action="edit" resource="${cbNodeInstance}">
