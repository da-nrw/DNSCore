				...
				<tr class="${ ((i % 2) == 0 ? 'odd' : 'even') + ' ' + statusType}">
					...
					<td>
						...
						<g:if test="${ queueEntryInstance.showRetryButtonAfterSomeTime()}">
							<g:link action="queueRetry" id="${queueEntryInstance.id}">
								<g:img style="width:16px; height:16px" uri="/images/icons/exchange32.png" title="${message(code: 'default.workflow.icon.retry', default: 'Workflow für Paket neu starten')}" alt="${message(code: 'default.workflow.icon.retry', default: 'Workflow für Paket neu starten')}"/>
							</g:link>
						</g:if>
						<g:if test="${queueEntryInstance.showRecoverButton() }">
							...
							<g:link action="queueRecover" id="${queueEntryInstance.id}">
								<g:img style="width:16px; height:16px" uri="/images/icons/back-icon.png" title="${message(code: 'default.workflow.icon.restart', default: 'Gesamten Workflow für Paket neu starten')}" alt="${message(code: 'default.workflow.icon.restart', default: 'Gesamten Workflow für Paket neu starten')}"/>
							</g:link>
						</g:if> 
						...
						<g:set var="showDeleteAll" value="true" />
							<g:link onclick="return confirm('Eintrag löschen. Sind Sie sicher?');" action="queueDelete" id="${queueEntryInstance.id}">
								<g:img style="width:16px; height:16px" uri="/images/icons/list_remove.png" title="${message(code: 'default.workflow.icon.delete', default: 'Paket löschen')}" alt="${message(code: 'default.workflow.icon.delete', default: 'Paket löschen')}"/>
							</g:link>
						</g:if>
					</td>
					<td>
			...
			<tr>
				...
				<input type="checkbox" onClick="toggle(this)"/>Alle an-/abwählen, für alle Pakete im Status ${params.search?.status}:
					...
					<g:if test="${ showRecoverAll == "true" }">
						<a onClick="return subForm('Den gesamten Workflow wiederholen? Sind Sie sicher?','../queueRecoverAll');">
							<img src="${resource(dir: 'images/icons', file: 'back-icon.png')}" style="width:16px; height:16px" title="${message(code: 'default.workflow.icon.restart', default: 'Gesamten Workflow für Paket neu starten')}" alt="${message(code: 'default.workflow.icon.restart', default: 'Gesamten Workflow für Paket neu starten')}"/></a>
					</g:if>
					<g:if test="${ showDeleteAll == "true" }">
						<a onClick="return subForm('Alle Pakete löschen. Sind Sie sicher?','../queueDeleteAll');" >
							<img src="${resource(dir: 'images/icons', file: 'list_remove.png')}" style="width:16px; height:16px" title="${message(code: 'default.workflow.icon.delete', default: 'Paket löschen')}" alt="${message(code: 'default.workflow.icon.delete', default: 'Paket löschen')}"></a>
					</g:if>
					</g:if>
				</td>
			</tr>
