					...
					<table class="table">
						<thead>
							<tr>
								<th class="sortable field-id">
									<a href="#" onClick="return sortQueue('queueEntry.obj.identifier');">
										${message(code: 'queueEntry.obj.identifier.label', default: 'Identifier')}
									</a>
								</th>
								<th class="sortable field-id">
									<a href="#" onClick="return sortQueue('queueEntry.question');">
										${message(code: 'queueEntry.question.label', default: 'Anfrage lautet')}
									</a>
								</th>
								<th class="sortable field-origName">Aktion</th>
								<th class="sortable field-urn">
									<a href="#" onClick="return sortQueue('urn');">
										${message(code: 'queueEntry.obj.urn', default: 'URN')}
									</a>
								</th>
								<th class="sortable field-contractorShortName">
									<a href="#" onClick="return sortQueue('contractorShortName');">
										${message(code: 'queueEntry.obj.user.shortName.label', default: 'Contractor')}
									</a>
								</th>
								<th class="sortable field-created">
									<a href="#" onClick="return sortQueue('created');">
										${message(code: 'queueEntry.created.label', default: 'Erstellt')}
									</a>
								</th>
								<th class="sortable field-modified">
									<a href="#" onClick="return sortQueue('modified');">
										${message(code: 'queueEntry.modified.label', default: 'Geändert')}
									</a>
								</th>
								<th class="sortable field-origName">
									<a href="#" onClick="return sortQueue('origName');">
										${message(code: 'queueEntry.obj.origName.label', default: 'Orig. Name')}
									</a>
								</th>
							</tr>
						</thead>
						<tbody>
								...
								<tr class="${ ((i % 2) == 0 ? 'odd' : 'even') + ' ' + statusType}">
									...
									<td>
										<g:link onclick="return confirm('Antwort Nein - Sind Sie sicher?');" action="performRequestNo" id="${queueEntryInstance.id}">
											<g:img style="width:16px; height:16px" uri="/images/icons/list_remove.png" title="${message(code: 'default.workflow.icon.no', default: 'Antwort Nein')}" alt="${message(code: 'default.workflow.icon.no', default: 'Antwort Nein')}" />
										</g:link> 
										<g:link onclick="return confirm('Antwort Ja - Sind Sie sicher?');" action="performRequestYes" id="${queueEntryInstance.id}">
											<g:img style="width:16px; height:16px" uri="/images/icons/check32.png" title="${message(code: 'default.workflow.icon.yes', default: 'Antwort Ja')}" alt="${message(code: 'default.workflow.icon.yes', default: 'Antwort Ja')}" />
										</g:link></td>
									<td>
							...
							<g:if test="${queueEntryInstanceList == null || queueEntryInstanceList.isEmpty()}">
								<tr class="even">
									<td colspan="8">
										<i>Es warten keine Anfragen des Systemsauf Ihre Entscheidung ...</i>
									</td>
								</tr>
							</g:if>
						</tbody>
