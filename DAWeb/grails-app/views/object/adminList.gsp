...
<g:set var="entityName"
	value="${message(code: 'object.label', default: 'Object')}" />
...
<body>
	<div class="page-body">
		...
		<a href="#list-object" class="skip" tabindex="-1"> <g:message
		...
		<div class="panel">
			<g:form name="searchForm" action="list">
				...
				<table class="abstand-oben">
					<tr>
						<td>Original Name:</td>
						<td><g:textField name="search.origName"
								value="${params.search?.origName}" size="50" class="input-hoehe" /></td>
					</tr>
					<tr>
						<td>URN:</td>
						<td><g:textField name="search.urn"
								value="${params.search?.urn}" size="50" class="input-hoehe" /></td>
					</tr>
					<tr>
						<td>Identifier:</td>
						<td><g:textField name="search.identifier"
								value="${params.search?.identifier}" size="50"
								class="input-hoehe" /></td>
					</tr>
					<tr>
						<td>Qualitätsstufe:</td>
						<td><g:select id="qualityLevel" name="searchQualityLevel"
								from="${['Stufe 1','Stufe 2','Stufe 3','Stufe 4','Stufe 5']}"
								keys="${['1','2','3','4','5']}"
								value="${params.searchQualityLevel}"
								noSelection="[null:'-Bitte wählen-']" /></td>
					</tr>
					<tr>
						<td>Datumsbereich:</td>
						<td><g:select id="datetype" name="searchDateType"
								from="${['Datum erstellt','Datum geändert']}"
								keys="${['createdAt','modifiedAt']}"
								value="${params.searchDateType}"
								noSelection="[null:'-Bitte wählen-']" /></td>
					</tr>
					<tr>
						<td>Von Datum:</td>
					...
					<g:if test="${admin}">
						<tr>
							<td>Contractor:</td>
							<td><g:if
									test="${params.searchContractorName  == null || params.searchContractorName.isEmpty()}">
									<g:select id="user" name="searchContractorName"
										from="${contractorList}" optionKey="shortName"
										noSelection="['':'-Bitte wählen-']"
										value="${objectInstance?.contractorList?.shortName}"
										class="many-to-one" />
								</g:if> <g:if
									test="${params.searchContractorName  != null && !params.searchContractorName.isEmpty()}">
									<g:select id="user" name="searchContractorName"
					...
					<tr>
						<td></td>
						<td><g:submitButton class="style-buttons" name="submit"
								value="Filter anwenden" /> <g:submitButton class="style-buttons"
								name="loeschen" type="submit" value="Filter löschen" /></td>
						<g:javascript>
			           			$(document).ready(function(){
		...
		<div id="list-object" class="content scaffold-list" role="main">
			<h3>Ihre "${objArt}" DA-NRW Objekte (${objectInstanceList.size()}
				Treffer von ${totalObjs} insgesamt)</h3>
				...
				<div style="overflow: auto; height: 600px">
					<table>
						<thead class="thead-line">
							<tr>
								<th><g:message code="object.identifier.label"
										default="Ident" /></th>
								<g:sortableColumn property="urn"
									title="${message(code: 'object.urn.label', default: 'Urn')}" />
								<g:sortableColumn property="user"
									title="${message(code: 'object.user.label', default: 'Contractor')}" />
								<g:sortableColumn property="origName"
									title="${message(code: 'object.origName.label', default: 'Orig Name')}" />
								<g:sortableColumn property="createdAt"
									title="${message(code: 'object.created.label', default: 'Erstellt')}" />
								<g:sortableColumn property="modifiedAt"
									title="${message(code: 'object.modified.label', default: 'Geändert')}" />
								<g:sortableColumn property="quality_flag"
								...
								<th style="text-align: center">Anfordern <g:if
										test="${!paginate}">
						...
						<tbody>
							<g:each in="${objectInstanceList}" status="i"
								var="objectInstance">
								...
								<tr class="${ ((i % 2) == 0 ? 'odd' : 'even')}">
									...
									<g:if test="${!objectInstance.isInWorkflowButton()}">
										...
										<td style="text-align: center"><g:remoteLink
												action="queueForRebuildPresentation"
												onLoaded="queuedFor(data)" id="${objectInstance.id}">
												<asset:image style="width:16px; height:16px"
													src="/icons/exchange32.png"
										...
										<td style="text-align: center"><g:remoteLink
												action="queueForIndex" onLoaded="queuedFor(data)"
												id="${objectInstance.id}">
												<asset:image style="width:16px; height:16px"
													src="/icons/exchange32.png"
									...
									<td><g:if test="${objectInstance.getPublished_flag()==1}">
										...
										</g:if> <g:if test="${objectInstance.getPublished_flag()==2}">
											<g:link url="${objectInstance.getInstPresLink()}"
												target="_blank">
												<asset:image width="16px" height="16px"
													src="/icons/globe.png" />
											</g:link>
									...
									<td style="text-align: center"><g:if
											...
											<g:link controller="outgoing" action="download"
												params="['filename':objectInstance.identifier +'.tar']">
												<asset:image style="width:16px; height:16px"
			...
			<g:if test="${paginate}">
				<!-- workaround weil paginate die search map zerhackstückelt - params="${searchParams}" -->
